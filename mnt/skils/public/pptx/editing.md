...
| 패키지 | 용도 |
|--------|------|
| `pptxgenjs` | PPTX 파일 생성 (슬라이드, 텍스트, 도형, 차트, 이미지) |
| `react` | react-icons의 의존성 (JSX 렌더링) |
| `react-dom` | react-icons SVG를 정적 마크업으로 변환 |
| `react-icons` | Font Awesome, Material Design 등 아이콘 라이브러리 |
| `sharp` | SVG → PNG 래스터라이즈 (아이콘 및 커스텀 배경 생성) |
...
| 패키지 | 용도 |
|--------|------|
| `markitdown[pptx]` | PPTX 텍스트 추출 (콘텐츠 QA용) |
| `Pillow` | 썸네일 그리드 생성 |
...
| Poppler (`pdftoppm`) | PDF → JPEG 이미지 변환 (슬라이드별) |
